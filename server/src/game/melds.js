...
// Проверить, что набор фишек образует корректную группу (комбинацию):
// сет (одинаковые значения, разные цвета) или серия (подряд идущие числа одного цвета).
// Дикие фишки (джокеры) могут заменять любую фишку.
export function isValidMeld(tiles) {
  const n = tiles.length;
  if (!Array.isArray(tiles) || n < 3 || n > 14) return false;

  const real = tiles.filter((t) => !isJoker(t));
  if (real.length === 0) return true; // только джокеры (длиной >= 3) — допустимо

  // Сет (одинаковые числа): все реальные фишки одного числа, разных цветов, максимум 4 фишки.
  ...
  // Серия: один цвет, неповторяющиеся значения, помещаются в окно длины n в пределах 1..13.
